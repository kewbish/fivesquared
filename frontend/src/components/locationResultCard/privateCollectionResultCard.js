function PrivateCollectionResultCard({ privateCollectionData }) {
		...
		<div className="flex flex-col justify-center max-w-xs p-6 shadow-md rounded-xl bg-gray-50 text-gray-900">
			...
			<div className="space-y-4 text-center divide-y dark:divide-gray-700">
				<div className="my-2 space-y-1">
					<h2 className="text-xl font-semibold sm:text-2xl">{privateCollectionData.name}</h2>
					<p className="px-5 text-xs sm:text-base text-gray-600">{privateCollectionData.city}, {privateCollectionData.country}</p>
				</div>
				<div className="flex justify-center pt-2 space-x-4 align-center">
					<a className="cursor-pointer underline text-gray-400" href={`/private-collection/${privateCollectionData.name}`}>View private collection</a>
				</div>
			</div>
		</div>
	);
...
export default PrivateCollectionResultCard;
