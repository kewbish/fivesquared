...
const Profile = () => {
    ...
    const fetchProfile = async () => {
      if (cookies["login_cookie"]) {
        const response = await fetch(
          `http://localhost:65535/profile/${cookies["login_cookie"]}/${tag}`,
          {
            method: "GET",
          }
        );
  ...
  const followToggle = async () => {
    if (following) {
      const response = await fetch(
        `http://localhost:65535/unfollow/${cookies["login_cookie"]}/${tag}`,
        {
          method: "DELETE",
        }
      );
      ...
      const response = await fetch(
        `http://localhost:65535/follow/${cookies["login_cookie"]}/${tag}`,
        {
          method: "POST",
        }
      );
  ...
  if (!cookies.login_cookie) {
    ...
    return (
      <div className="text-center flex flex-col bg-white border shadow-sm rounded-xl p-6">
        <h2 className="font-bold text-gray-800">Loading profile data...</h2>
      </div>
    );
  }
};
