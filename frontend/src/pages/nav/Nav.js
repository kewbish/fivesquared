...
const Nav = () => {
  ...
  return (
    <>
<nav
        className="flex items-center justify-between flex-wrap bg-white py-4 lg:px-12 shadow border-solid border-t-2 border-blue-500">
        ...
        <div className="menu w-full flex-grow lg:flex lg:items-center lg:w-auto lg:px-3 px-8">
            ...
            <div className="relative mx-auto text-gray-600 lg:block hidden">
                ...
                <input
                    onKeyDown={handleKeyPress} onChange={(e) => setSearchTerm(e.target.value.toUpperCase())} className="border-2 border-gray-300 bg-white h-10 pl-2 pr-8 rounded-lg text-sm focus:outline-none"
                    type="search" name="search" placeholder="Search"/>
                <button onClick={search} type="submit" className="absolute right-0 top-0 mt-3 mr-2">
            ...
            <div className="flex ">
                {!loggedIn ? <a href="/login"
                   className="block text-md px-4 py-2 rounded text-blue-500 ml-2 font-bold hover:text-white mt-4 hover:bg-blue-500 lg:mt-0">Sign
                    in</a> : <></>}
                    {!loggedIn ? <a href="/signup"
                   className="block text-md px-4 py-2 rounded text-blue-500 ml-2 font-bold hover:text-white mt-4 hover:bg-blue-500 lg:mt-0">Sign
                    up</a> : <></>}
                    {loggedIn ? <a href={cookies[cookieName]}
                   className="block text-md px-4 py-2 rounded text-blue-500 ml-2 font-bold hover:text-white mt-4 hover:bg-blue-500 lg:mt-0 cursor:pointer">{"Welcome, " + cookies[cookieName] + "!"}</a> : <></>}
                    {loggedIn ? <a onClick={onLogOutClick}
                   className="block text-md px-4 py-2 rounded text-blue-500 ml-2 font-bold hover:text-white mt-4 hover:bg-blue-500 lg:mt-0 cursor:pointer">Log
                    out</a> : <></>}
