...
const Location = () => {
  ...
  const fetchLocation = async () => {
    const response = await fetch(`http://localhost:65535/location/${cookies["param_cookie"]}`, {
      method: "GET",
    });
    const pjson = await response.json();
    console.log("Collection Page PJSON", pjson);
    setLocationData(pjson.location);
  };

  const getPosts = async () => {
    const response = await fetch(`http://localhost:65535/posts/location/${cookies["param_cookie"]}`, {
      method: "GET",
    });
    const pjson = await response.json();
        ...
        <div class="p-16">
          <div class="p-8 bg-white border shadow-sm rounded-xl mt-24">
            ...
            <div class="mt-20 text-center pb-12">
              <h1 class="text-4xl font-medium text-gray-700">{locationData.name}, <span class="font-light text-gray-500">{locationData.city}, {locationData.country}</span></h1>
              <p class="font-light text-gray-600 mt-3">{locationData.st_address}<br/>{locationData.city}, {locationData.region} <br/> {locationData.country}&nbsp;{locationData.postcode}</p>
            </div>
