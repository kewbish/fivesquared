...
const Museum = () => {
  ...
  const fetchMuseum = async () => {
    const response = await fetch(`http://localhost:65535/museum/${name}`, {
      method: "GET",
    });
    const pjson = await response.json();
    console.log("Museum Page PJSON", pjson);
    setMuseumData(pjson.museum);
  };
          ...
          <div className="p-8 bg-white border shadow-sm rounded-xl mt-24">
            <div className="grid grid-cols-1 md:grid-cols-3">
              <div className="grid grid-cols-3 text-center order-last md:order-first mt-20 md:mt-0">
                <div>
                  <p className="font-bold text-gray-700 text-xl">{museumData.num_visitors}</p>
                  <p className="text-gray-400">No. Visitors</p>
                </div>
                <div>
                  <p className="font-bold text-gray-700 text-xl">{museumData.yr_est}</p>
              ...
              <div className="grid grid-cols-2 text-center mt-20 md:mt-0">
                <div>
                  <p className="font-bold text-gray-700 text-xl">{museumData.benefactor}</p>
                  <p className="text-gray-400">Benefactor</p>
                </div>
                <div>
                  <p className="font-bold text-gray-700 text-xl">${museumData.ticket_price} CAD</p>
            ...
            <div className="mt-20 text-center pb-12">
              <h1 className="text-4xl font-medium text-gray-700">{museumData.name}</h1>
              <p className="text-2xl font-light text-gray-500 mt-3">{museumData.st_address}<br/>{museumData.city}, {museumData.region} {museumData.postcode}<br/> {museumData.country}</p>
            </div>
